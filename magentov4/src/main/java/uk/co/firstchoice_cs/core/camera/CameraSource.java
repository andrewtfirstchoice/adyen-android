...
package uk.co.firstchoice_cs.core.camera;
...
public class CameraSource {
  ...
  private BarCodeRecognitionProcessor frameProcessor;
  ...
  public void release() {
    ...
    {
      LoggingHelper.errorMsg("CameraSource:Release",ex.getMessage());
    }
  }
  ...
  public void switchOnTorch(boolean flashOn)
  {
    Camera.Parameters p = camera.getParameters();
    if(flashOn)
  ...
  public synchronized void stop() {
    ...
    if (processingThread != null) {
      try {
        ...
        LoggingHelper.debugMsg(TAG, "Frame processing thread interrupted on release.");
      }
    ...
    if (camera != null) {
      ...
      try {
        ...
        Log.e(TAG, "Failed to clearScans camera preview: " + e);
      }
      camera.release();
      camera = null;
    }
  ...
  /**
   * Generates a nav_manuals of acceptable preview sizes. Preview sizes are not acceptable if there is not
  ...
  public void setMachineLearningFrameProcessor(BarCodeRecognitionProcessor processor) {
    synchronized (processorLock) {
      cleanScreen();
      if (frameProcessor != null) {
        frameProcessor.stop();
    ...
    void setNextFrame(byte[] data, Camera camera) {
      synchronized (lock) {
        ...
        if (!bytesToByteBuffer.containsKey(data)) {
          LoggingHelper.debugMsg(TAG, "Skipping frame. Could not find ByteBuffer associated with the image " + "data from the camera.");
          return;
        }
        ...
        synchronized (lock) {
          while (active && (pendingFrameData == null)) {
            try {
              ...
              LoggingHelper.debugMsg(TAG + "Frame processing loop terminated.", e.toString());
              return;
            }
          }
          ...
          // below.  We need to clearScans pendingFrameData to ensure that this buffer isn't
