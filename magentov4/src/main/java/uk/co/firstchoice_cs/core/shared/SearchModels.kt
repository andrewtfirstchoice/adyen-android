...
import kotlinx.android.synthetic.main.search_models.*
...
open class SearchModels : Fragment(R.layout.search_models), KoinComponent, SearchView.OnQueryTextListener {
    ...
    private val filterList: ArrayList<Document> = ArrayList()
    ...
    override fun onCreateOptionsMenu(menu: Menu, menuInflater: MenuInflater) {
        ...
        searchEditText.hint = "Filter models"
    ...
    private fun goBack()
    {
        NavHostFragment.findNavController(this@SearchModels).navigateUp()
    }
    ...
    inner class DocumentAdapter : RecyclerView.Adapter<DocumentAdapter.DocumentViewHolder>() {
        ...
        fun initList() {
            val documents = mViewModel.documents
            if (documents != null)
                filterList.addAll(documents)
        }
        ...
        fun filterResults(searchWords: String) {
            ...
            for (searchWord in searchTerms) {
                if (man != null)
                    for (entry in man) {
                        val fccPart =  entry.fccPart
                        if(fccPart!=null)
                        {
                            if (fccPart.contains(searchWord, ignoreCase = true)) {
        ...
        override fun onBindViewHolder(holder: DocumentViewHolder, position: Int) {
            ...
            holder.title.text = item.fccPart
            holder.itemView.setOnClickListener {
                mViewModel.selectedModel = item
                setFragmentResult(REQUEST_KEY, bundleOf("data" to item.classId))
                goBack()
            }
        }
    ...
    companion object {
        const val REQUEST_KEY: String = "SearchModels"
    }
}